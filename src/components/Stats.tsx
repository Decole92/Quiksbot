...
import { Users, Mail, BarChart3, Inbox, Loader2, Loader } from "lucide-react";
...
export default function Stats() {
  ...
  return (
    <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4 mb-4 w-full '>
      {campaignLoading || customerLoading ? (
        <div className='col-span-full flex items-center justify-center w-full h-full'>
          <Loader className='animate-spin w-6 h-6 text-[#E1b177] dark:text-gray-900 fill-gray-100' />
        </div>
      ) : (
        <>
          {stats.map((stat) => (
